package net.bi83.bonappetit.core.common.ui;
...
public class CookingPotTooltip implements ClientTooltipComponent {
    ...
    public int getWidth(Font font) {
        if (!mealStack.isEmpty()) {
            MutableComponent textServingsOf = mealStack.getCount() == 1
                    ? TextHelper.getTranslation("tooltip.cooking_pot.single_serving")
                    : TextHelper.getTranslation("tooltip.cooking_pot.many_servings", mealStack.getCount());
            return Math.max(font.width(textServingsOf), font.width(mealStack.getHoverName()) + 20);
        } else {
            return font.width(TextHelper.getTranslation("tooltip.cooking_pot.empty"));
        }
    }
    ...
    public void renderText(Font font, int x, int y, Matrix4f matrix4f, MultiBufferSource.BufferSource bufferSource) {
        ...
        if (!mealStack.isEmpty()) {
            ...
            MutableComponent textEmpty = TextHelper.getTranslation("tooltip.cooking_pot.empty");
